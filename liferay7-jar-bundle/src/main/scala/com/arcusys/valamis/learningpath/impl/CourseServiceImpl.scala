...
import com.liferay.portal.kernel.model.{Group, GroupConstants}
  ...
  override def getCourseTitlesByIds(ids: Seq[Long]): Future[Map[Long, String]] = Future {
    ids flatMap { id =>
      Option(GroupLocalServiceUtil.fetchGroup(id)) map { g =>
        (g.getGroupId, g.getDescriptiveName(Locale.getDefault))
      }
    } toMap
  }

  override def getCourseById(id: Long) = Future {
    implicit val locale = Locale.getDefault//TODO use user locale

    Option(GroupLocalServiceUtil.fetchGroup(id))
      .map(extract)
  }

  private def extract(group: Group)
                     (implicit locale: Locale): Course = {
    Course(group.getGroupId, group.getDescriptiveName(locale), getCourseFriendlyUrl(group))
  }
