...
/**
  ...
  */
class LevelChangeListener(val recommendedCompetenceService: CompetenceService,
  ...
  override def processMessage(data: String)
    ...
    msg.action match {
      case LevelChanged | LevelDeleted =>
        //if level is deleted, it's not deleted from LPs, but we still update its name
        //just in case it wasn't updated before
        await {
          for {
            _ <- improvingCompetenceService.updateLevelName(msg.id, msg.name)
            _ <- recommendedCompetenceService.updateLevelName(msg.id, msg.name)
          } yield None
        }
