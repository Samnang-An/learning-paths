...
class SlickTestProperties(dbName: String, properties: Properties) {
  ...
  lazy val testUserName = Option(
    Option(properties.getProperty("testUsername")).getOrElse(userName)
  ).map(_.replace("{{DB_NAME}}", dbName)).orNull//for Oracle

  lazy val testPassword = Option(properties.getProperty("testPassword")).getOrElse(password)
